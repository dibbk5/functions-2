...
const contains = (arr, oneName, callback) => {
  if(arr.includes(oneName)) {
    callback(true)
  } else {
    callback(false)
  }
...
uniq(names, uniqarrparam => console.log(`The new names array with all the duplicat items removed is ${uniqarrparam}`))
...
const each = (arr, callback) => arr.forEach((nme, ind) => callback(nme,ind))
